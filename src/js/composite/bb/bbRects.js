...
const bbRects = (state = {}) => {
  ...
  self.fnBefore((s) =>
    s
      .attr("x", self.fnDoubleBandCenterOut()(0))
      .attr("width", 0)
  ...
  self.fnAfter((s) =>
    s
      .attr(
        "x",
        (d, i, nodes) =>
          +select(nodes[i]).attr("x") + +select(nodes[i]).attr("width") / 2
      )
      .attr("width", 0)
      .attr(
        "y",
        (d, i, nodes) =>
          +select(nodes[i]).attr("y") + +select(nodes[i]).attr("height") / 2
      )
      .attr("height", 0)
      .attr("opacity", 0)
  );
