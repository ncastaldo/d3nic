...
const geoRegions = (state = {}) => {
  const self = pipe(component, hasGeoOut, hasMultiDrawFactory("path"))(state);

  self.fnBefore((s) => s.attr("d", self.fnGeoOut()).attr("opacity", 0));

  self.fnNow((s) => s.attr("d", self.fnGeoOut()));
