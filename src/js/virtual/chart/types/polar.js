...
const hasPolar = (state = {}) => {
  ...
  const update = () => {
    ...
    center = [mean(extent, (e) => e[0]), mean(extent, (e) => e[1])];
    radius = Math.min(center[0] - extent[0][0], center[1] - extent[0][1]);
  ...
  const draw = () => {
    ...
    if (firstDraw) {
      self.group().attr("transform", translate);
    } else {
      self.group().transition(self.transition()).attr("transform", translate);
    }
