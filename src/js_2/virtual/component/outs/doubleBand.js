const hasDoubleBandOut = (state = {}) => {
  ...
  const updateOuts = (chart) => {
    ...
    fnDoubleBandLeftOut = k => (d, i) => Math.max(
      ...
      fnDoubleBandOut(k)(d, i) - fnDoubleBandScales[k].step() * fnDoubleBandScales[k].paddingInner() / 2
    )
    fnDoubleBandRightOut = k => (d, i) => Math.min(
      fnDoubleBandOut(k)(d, i) + doubleBandwidthOut(k) + fnDoubleBandScales[k].step() * fnDoubleBandScales[k].paddingInner() / 2,
